...
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
gcc -O0 -fno-stack-protector -no-pie -o "$tmp/aurad-parser" "$here/src/aurad-parser.c"
echo 'AURA{healthcheck_local}' > "$tmp/flag.txt"
cp "$here/solution/exploit.py" "$tmp/exploit.py"
( cd "$tmp" && python3 exploit.py ) | grep -q 'AURA{healthcheck_local}'   && echo "OK: aurad-parser exploit lands the flag"   || { echo "FAIL: aurad-parser exploit did not recover the flag"; exit 1; }
